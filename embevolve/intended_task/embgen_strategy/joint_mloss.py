...
def train_pinsage_penalize(model, device, train_loader, optimizer, train_config_dict, x_user_prev_model, x_item_prev_model, args, trans_fun, optimizer_trans, best_trans_fun_list):
    model.train()
    trans_fun.train()
    ...
    for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
        ...
        diff_user = x_user_prev_model[user] - trans_fun(model.x_user[user])
        diff_item = x_item_prev_model[item] - trans_fun(model.x_item[item])
        diff_item_neg = x_item_prev_model[item_neg] - trans_fun(model.x_item[item_neg])

        loss_penalty = torch.mean(diff_user**2) + torch.mean(diff_item**2) + torch.mean(diff_item_neg**2)

        for best_trans_fun in best_trans_fun_list[::-1]:
            diff_user = F.linear(diff_user, best_trans_fun.weight)
            diff_item = F.linear(diff_item, best_trans_fun.weight)
            diff_item_neg = F.linear(diff_item_neg, best_trans_fun.weight)
            loss_penalty += torch.mean(diff_user**2) + torch.mean(diff_item**2) + torch.mean(diff_item_neg**2)

        loss_penalty = loss_penalty / (len(best_trans_fun_list) + 1)
...
def train_eval_loop_pinsage_penalize(args, model, device, train_dataset, train_loader, optimizer, train_config_dict,\
                                eval_dict, K_list, K_primary, time_train, time_val, writer, x_user_prev_model, x_item_prev_model, trans_fun, optimizer_trans, best_trans_fun_list):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('Training...')
        train_dataset.sample_neg_batch() # sample a new set of negative items in each training epoch
        train_loss = train_pinsage_penalize(model, device, train_loader, optimizer, train_config_dict, x_user_prev_model, x_item_prev_model, args, trans_fun, optimizer_trans, best_trans_fun_list)
...
def joint_mloss(args):
    ...
    for i in range(len(sliced_time_list)-1):
        ...
        if i == 0:
            ...
            # not learning trans_fun once it is fixed
