...
def train_trans(trans_fun, device, train_loader, optimizer_trans, old_embedding, new_embedding, args, best_trans_fun_list):
    ...
    for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
        ...
        diff_user = emb_user_old[user] - trans_fun(emb_user_new[user])
        diff_item = emb_item_old[item] - trans_fun(emb_item_new[item])
        diff_item_neg = emb_item_old[item_neg] - trans_fun(emb_item_new[item_neg])
...
def train_eval_loop_trans(args, trans_fun, device, train_dataset, train_loader, optimizer_trans,\
                             old_embedding, new_embedding, time_train, time_val, writer, best_trans_fun_list):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_trans(trans_fun, device, train_loader, optimizer_trans, old_embedding, new_embedding, args, best_trans_fun_list)
...
def posthoc_mloss(args):
    print(f'Posthoc training of Bk ({args.trans_type}) with multi-step alignment loss.')
    print(args)
